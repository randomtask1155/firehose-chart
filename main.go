...
var (
	...
	mCollectors    = flag.String("m", "", "Json array of metrics [{ \"origin\": \"bosh-system-metrics-forwarder\", \"type\": \"ValueMetric\", \"job\": \"router\", \"index\": \"e5403b4a-646e-4f5a-a195-2356b3116c21\", \"metric\": \"system.cpu.user\"} ]")
	maxSeries      = flag.Int("max", 100, "Max number of sample to display in chart")
	logger         *log.Logger
	cfconf         CFConfig
	mc             []Metric
...
func (c *CFConfig) setDopplerEndpoint() error {
	...
	c.WSSURL = info.DopplerEndpoint + "/firehose/firehose-chart"
	return nil
}
...
func setupMetrics() {
	...
	if *mCollectors == "" {
		logger.Fatalln("you must define what metrics to collect with \"-m origin:type:job:metric,origin:type:job:metric\" ")
	}

	err := json.Unmarshal([]byte(*mCollectors), &mc)
	if err != nil {
		logger.Fatalf("could not parse -m: %s", err)
	}
